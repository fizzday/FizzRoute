...
class Route
{
    ...
    public static function dispatch()
    {
        ...
        // 未匹配到路由(run the error callback if the route was not found)
...
//        }
        // 自动路由
        if ($found_route == false) {
            if (!$uri) (new \App\Controllers\HomeController())->index();
            ...
            switch ($count) {
                case 1:
                    $action = '\\App\\Controllers\\' . ucfirst($uri) . 'Controller';
                    (new $action())->index(self::$request);
                    break;

                default:
                    $function = array_pop($uri_arr);
                    $controller = array_pop($uri_arr);
                    $dir = str_replace(['/'.$function, $controller], '', $uri);
                    $dir = str_replace(['/'], '\\', $dir);
                    $action = '\\App\\Controllers\\' . $dir . ucfirst($controller) . 'Controller';

                    (new $action())->$function(self::$request);
                    break;
            }
        }
    }
    ...
    private static function matchController($index, $param=[])
    {
        ...
        $fileName = (self::getControllerNamespace()) . '\\' . $segments[0];
        $controller = new $fileName();
        $func = $segments[1];

        //call function
        $controller->$func(self::$request);
    }

    private static function getControllerNamespace()
    {
        return config('config.namespace.controller', '\\App\\Controllers');
    }
}